...
class UserControllerTest {
	...
	void findAll() throws Exception {
		//given
		List<UserSearchDto> reponse = List.of(new UserSearchDto("id1", "password"),
			new UserSearchDto("id2", "password2"));
		given(userService.findAll()).willReturn(reponse);

		//expect
		mockMvc.perform(get("/api/user"))
			.andExpect(status().isOk())
			.andExpect(jsonPath("$.message").value("성공적으로 유저가 조회되었습니다."))
			.andExpect(jsonPath("$.entity[0].id").value("id1"))
			.andExpect(jsonPath("$.entity[0].password").value("password"))
			.andExpect(jsonPath("$.entity[1].id").value("id2"))
